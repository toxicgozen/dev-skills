...
[`ask-matt`](./skills/engineering/ask-matt/SKILL.md) is the router that maps every promoted, user-reachable skill and how they relate. The same trigger that re-syncs a docs page applies to it: whenever you add, rename, remove, or change how a promoted, user-reachable skill fits the flows, re-read `ask-matt`'s `SKILL.md` and update it so the map stays accurate — a promoted skill it never mentions, or a stale one it still routes to, is a router that lies. Skills in non-promoted buckets stay out of both the router and human-facing docs.
