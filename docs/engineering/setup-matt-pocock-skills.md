...
- **Domain docs** — assumed single-context (one `CONTEXT.md` + `docs/adr/` at the root), which fits almost every repo; it only raises a multi-context map when it spots monorepo signals. The generated consumer rule treats every concept recorded in `CONTEXT.md` as canonical instead of asking downstream skills to classify it again.
