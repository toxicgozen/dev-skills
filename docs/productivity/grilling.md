...
It asks **one question at a time** and waits for your answer before the next — never a bulk list, which is bewildering. Each question comes with the agent's own recommended answer, and any question the codebase can settle it explores instead of asking you. Before introducing or changing a framework, record shape, policy, or claimed best practice, it checks relevant primary sources unless the choice is explicitly local and outside practice cannot change it; its own synthesis stays labelled as such. It won't start enacting the plan until you confirm the shared understanding has been reached.
